...
/* Do BP */
int bp(NETWORK G, int k_comm, int *x, double **gmma, double **omega, double ***eta, double **q)
{
  int i,j;
  int u,v;
  int r,s;
  ...
  do
  {
	...
	for (r=0; r< k_comm; r++)
	  ...
	  for (j=0; j<G.vertex[v].degree; j++) 
	    ...
	    if (G.vertex[v].edge[j].target!=u)
	      ...
	      sum = 0.0;
	      for (s=0; s< k_comm; s++) 
                sum += eta[v][j][s]*omega[r][s];
	      if (sum<SMALL) 
                sum = SMALL;   // Prevent -Inf
	      logeta += log(sum);
	    }
	  }
	...
	for (r=1; r< k_comm; r++) 
        {
	  if (logetaun[u][i][r]>largest) 
            largest = logetaun[u][i][r];
	}
	for (r=0; r< k_comm; r++) 
        {
